...
<body>
  ...
  <header>
    ...
    <nav class="navbar fixed-top navbar-expand-lg bg-body-tertiary">
     ...
     <!--close--> 
    <!--Table 1-->
    <h1 class="display-1" style="text-align: center;">Common Trip</h1>
  
<?php
    include 'dbconnect.php';
    $sql="SELECT * FROM common_trips ";
...
<?php
    ...
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
        ...
        echo "<h5 class='card-title'>Company Id:{$row['company_id']}</h5>";
        echo "<h5 class='card-title'>Station:{$row['station']}</h5>";
        echo "<h5 class='card-title'>Transportation Place:{$row['transportation_place']}</h5>";
        echo "<h5 class='card-title'>Price :{$row['price']} OMR</h5>";
        echo "<h5 class='card-title'>Note : {$row['massage']}</h5>";
        echo '<br>';
        $url = "passengerbook.php?trip_id={$row['trip_id']}&date={$row ['date']}";

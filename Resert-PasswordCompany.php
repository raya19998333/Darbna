...
if(isset($_REQUEST['pwdrst']))
{
  ...
  if($pwd == $cpwd)
  {
    $sql="UPDATE companytransport_info SET Password='$pwd' where Email='$email'";
    $reset_pwd = mysqli_query($conn,$sql);
    if($reset_pwd>0)
    {
      $msg = 'Your password updated successfully <a href="companyLogin.php">Click here</a> to login';
    }
    else
    {
...
if($_GET['secret'])
{
  ...
  $check_details = mysqli_query($conn,"SELECT Email FROM companytransport_info where Email='$email'");

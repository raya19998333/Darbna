...
</header><br><br><br><br><h1 class="display-2" style="text-align: center;">Special Trip</h1> <br><br><br>

    <?php
          include 'dbconnect.php';
          $sql="SELECT * FROM special_trip ";
          if(!$result=$conn->query($sql)){
              die("There was an error running the query".$conn->error);
           }
          ?>
        <div class="card-container"  id="p">
<?php
    $count = 0;
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
        ...
        echo '<div class="col" >';
        echo '<div class="card text-center" i >';
        echo '<div class="card-header" >';
        echo "{$row ['transportation_place']}";
        echo "</div>";
        echo '<div class="card-body" >';
        echo "<h5 class='card-title'>Trip Id :{$row ['trip_id']}</h5>";
        echo "<h5 class='card-title'>Number of passenger :{$row ['passenger_number']}</h5>";
        echo "<h5 class='card-title'>Location :{$row ['location']}</h5>";
        echo "<h5 class='card-title'>Vehicle Type :{$row ['vehicle_type']}</h5>";
        echo "<h5 class='card-title'>Place :{$row ['transportation_place']}</h5>";
        echo "<h5 class='card-title'>Price :{$row ['price']} OMR</h5>";

        echo '<br>';

        echo "<a href='passengerbook.php?trip_id={$row['trip_id']}&date={$row ['date']}' class='btn btn-success'>Booking</a>";
        echo '</div>';
